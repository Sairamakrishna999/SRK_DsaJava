	...
	public class BuyTheTicket {
		public static int buyTicket(int input[], int k) {
			...
			while (!queue.isEmpty()) {
				...
				if (priorityQueue.peek() == queue.peek().value) // if the element at the start of the queue is same as max
																// of priority queue
				{
					...
					count++; // add 1 to time
					if (pair.index == k) { // if removed index is same as index passed to the function, return else
											// continue
						break;
					}
				} else {
